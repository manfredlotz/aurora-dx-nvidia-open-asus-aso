# Custom image aurora-dx-nvidia-open-asus-aso

This repository is a fork from <https://github.com/ublue-os/image-template> which explains
nicely how to build a custom image.

The image _aurora-dx-nvidia-open-asus-aso_ is based on ghcr.io/ublue-os/aurora-dx-nvidia-open:stable

- `asus` indicates that the image contains the asus software which is required to be able to define a charge limit for the battery
- `aso` means _and so on_ and indicates that additional software was added.

Here the list of additional software installed

- asusctl and asusctl-rog-gui
